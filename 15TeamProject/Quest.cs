...
namespace _15TeamProject
{
    ...
    public static class QuestDB
    {
        ...
        public static List<Quest> questList = new List<Quest>()
        {
            ...
            new Quest("마을을 위협하는 미니언 처치", "이봐! 마을 근처에 미니언들이 너무 많아졌다고 생각하지 않나? \n마을 주민들의 안전을 위해서라도 저것들 수를 좀 줄여야 한다고! \n모험가인 자네가 좀 처치해주게!", "미니언 다섯 마리 처치", "미니언", 5, 500, 15, "낡은 검 x 1", 3),
            new Quest("위험한 대포를 지닌 미니언 처치", "근래에 미니언들 중에서 대포를 가지고 다니는 녀석들이 나타났다고 들었네, 그들이 마을의 목책을 부수진 않을까 걱정이야. \n마을의 안전을 위해 그들을 처치해 줄 수 있겠나?", "대포 미니언 3마리 처치", "대포미니언", 3, 1000, 30, "청동 도끼 x 1", 4),
            new Quest("테스트용 퀘스트", "퀘스트 테스트로 공허충 1마리만 잡고 변경사항 확인해보기","공허충 1마리 잡기", "공허충", 1, 1000, 20, "스파르타의 창 x 1", 5 )
        };
    }
}
    ...
    ////퀘스트 완료시 아이템 추가하는 메소드. 
    //    public static void AddItem(int itemIndex)
    //    {
    //        if (itemIndex >= 0 && itemIndex < ItemDB.ItemList.Count)
    //        {
    //            inventory.Add(itemIndex + 1);   // 인덱스 0부터 시작해서 + 1 해줌
    //            Console.WriteLine($"[아이템 보상] \'{ItemDB.ItemList[itemIndex].ItemNames}\'이(가) 인벤토리에 추가되었습니다!");
    //        }
    //        else
    //        {
    //            Console.WriteLine("잘못된 아이템 인덱스입니다. 아이템을 추가할 수 없습니다.");
    //        }
    //    }
    //
